package io.pivotal.samples.dashboard.repositories;
...
@Component
public class ClientRepositoryPopulator implements ApplicationListener<ContextRefreshedEvent>, ApplicationContextAware {
    private final Jackson2ResourceReader resourceReader;
    private final Resource sourceData;

    private ApplicationContext applicationContext;

    public ClientRepositoryPopulator() {
        ObjectMapper mapper = new ObjectMapper();
        mapper.configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false);
        resourceReader = new Jackson2ResourceReader(mapper);
        sourceData = new ClassPathResource("clients.json");
    }
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        if (event.getApplicationContext().equals(applicationContext)) {
            ...
            if (clientRepository != null && clientRepository.count() == 0) {
                populate(clientRepository);
            }
        }
    ...
    public void populate(CrudRepository repository) {
        ...
        if (entity instanceof Collection) {
            for (Client client : (Collection<Client>) entity) {
                if (client != null) {
